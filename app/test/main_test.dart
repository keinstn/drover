...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // logic.
      //
      // This test stops at `changes` emitting on the exact instance
      // `_DroverAppState` holds — it does NOT exercise passing
      // `_staleTransportSignal.changes` into `HerdScreen`'s `networkChanges`
      // (the `_registry.invalidateAll()` subscription is covered separately
      // below via an injected registry). Proving the `HerdScreen` wiring
      // would need a rendered `HerdScreen` with hosts configured and an
      // existing failure/backoff state to observe clearing, which is out of
      // this unit's scope per the contract.
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // emitted — proving `initState`'s
      // `_staleTransportSignal.changes.listen(... _registry.invalidateAll
      // ...)` subscription actually reached the injected registry, not just
      // that the signal itself fired (covered separately above).
      await tester.pump(const Duration(milliseconds: 30));
