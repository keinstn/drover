...
- **プッシュ通知バックエンド** — 開発者が運用する唯一のサービスです。エージェントが返事を待っていることをデバイスに伝えるほか、音声アシスタントのクレジット残高も管理しています。
...
プッシュ通知は**任意**です。動作を開始するのはホストをペアリングした後だけで、ペアリングはお客様が意図して行う操作です。ホストをペアリングせず、Apple でサインインもしなければ、お客様に関するデータがバックエンドに保存されることはまったくありません。
...
| **音声セッションの記録** — アカウント識別子と通話が始まった時刻。お使いのデバイスが通話ごとに作るセッション ID の下に保存される | 5分の通話の中で接続し直しても、1回の通話として一度だけ課金するため | データを削除しても消えない（自動削除の仕組みは今のところない） |
